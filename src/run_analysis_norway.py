...
def load_norway_data_github():
    """Télécharge les données COVID-19 Norvège depuis GitHub JHU CSSE."""
    url = "https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_deaths_global.csv"

    print("📥 Téléchargement des données Norvège depuis GitHub JHU CSSE...")
    try:
        df = pd.read_csv(url)
        # Norvège - somme de toutes les régions
        norway_data = df[df['Country/Region'] == 'Norway'].iloc[:, 4:].sum(axis=0)
        norway = pd.DataFrame({'deaths': norway_data})
        norway.index = pd.to_datetime(norway.index)

        # Filtrer Vague 1
        start_date = '2020-02-15'
        end_date = '2020-06-30'
        norway = norway.loc[start_date:end_date]

        # Calculer décès quotidiens
        daily_deaths = norway['deaths'].diff().fillna(0)
        ...
        print(f"✅ Données Norvège chargées: {len(t_data)} points")
...
def plot_comparison(t_data, y_data, dates, models_dict, output_path):
    ...
    fig.suptitle('COVID-19 Norvège Vague 1 - Comparaison Super-Radiant vs SIR\n(Confinement STRICT - Test vs Suède)',
    ...
    if models_dict['sr4'] is not None:
        ...
        mode_names = ['Mode 1 (Oslo)', 'Mode 2 (Villes)', 'Mode 3 (Régional)', 'Mode 4 (Nord)']
...
def main():
    ...
    print("=" * 70)
    print("  ANALYSE COVID-19 NORVÈGE VAGUE 1")
    print("  Super-Radiant (sech²) vs SIR Standard")
    print("  TEST CRITIQUE: Norvège (strict) vs Suède (volontaire)")
    print("=" * 70)
    print()

    # [1] Chargement des données
    print("[1/5] Chargement des données Norvège...")
    t_data, y_data, dates, max_deaths_raw = load_norway_data_github()
    print()
    ...
    if best_model and sir_model:
        ...
        print(f"📊 RÉSULTATS NORVÈGE VAGUE 1:")
    ...
    if improvement < 1.0:
        print("   │ Norvège    │ Strict (12 mars)│ SR       │ RMS {:.2f}%  │".format(best_rms))
    else:
        print("   │ Norvège    │ Strict (12 mars)│ SIR      │ RMS {:.2f}%  │".format(rms_percent_sir))
    print("   └────────────┴─────────────────┴──────────┴─────────────┘")
    print()
    if improvement > 1.0:
    ...
    print()
    print("✅ Analyse Norvège terminée!")
    print("=" * 70)
