#!/usr/bin/env python3
"""
Analyse COVID-19 Suisse - Première Vague
Comparaison Super-Radiant (sech²) vs SIR Standard

CAS INTÉRESSANT: Petit pays fédéral avec forte autonomie cantonale
...
def load_switzerland_data_github():
    """Télécharge les données COVID-19 Suisse depuis GitHub JHU CSSE."""
    url = "https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_deaths_global.csv"

    print("📥 Téléchargement des données Suisse depuis GitHub JHU CSSE...")
    try:
        df = pd.read_csv(url)
        # Suisse - somme de toutes les régions
        switzerland_data = df[df['Country/Region'] == 'Switzerland'].iloc[:, 4:].sum(axis=0)
        switzerland = pd.DataFrame({'deaths': switzerland_data})
        switzerland.index = pd.to_datetime(switzerland.index)

        # Filtrer Vague 1
        start_date = '2020-02-15'
        end_date = '2020-06-30'
        switzerland = switzerland.loc[start_date:end_date]

        # Calculer décès quotidiens
        daily_deaths = switzerland['deaths'].diff().fillna(0)
        ...
        print(f"✅ Données Suisse chargées: {len(t_data)} points")
        ...
        print(f"   📍 Structure: Pays fédéral (26 cantons), autonomie forte")
...
def plot_comparison(t_data, y_data, dates, models_dict, output_path):
    ...
    fig.suptitle('COVID-19 Suisse Vague 1 - Comparaison Super-Radiant vs SIR\n(Pays fédéral - 26 cantons)',
    ...
    if models_dict['sr4'] is not None:
        ...
        mode_names = ['Mode 1 (Zurich/Genève)', 'Mode 2 (Cantons urbains)', 'Mode 3 (Régional)', 'Mode 4 (Alpin)']
    ...
    ax3.set_title('Décomposition en 4 modes cantonaux', fontsize=12, fontweight='bold')
...
def main():
    ...
    print("=" * 70)
    print("  ANALYSE COVID-19 SUISSE VAGUE 1")
    print("  Super-Radiant (sech²) vs SIR Standard")
    print("  Pays fédéral - 26 cantons autonomes")
    print("=" * 70)
    print()

    # [1] Chargement des données
    print("[1/5] Chargement des données Suisse...")
    t_data, y_data, dates, max_deaths_raw = load_switzerland_data_github()
    print()
    ...
    if best_model and sir_model:
        ...
        print(f"📊 RÉSULTATS SUISSE VAGUE 1:")
    ...
    output_path = reports_dir / "comparison_switzerland_wave1.png"
    plot_comparison(t_data, y_data, dates, models_dict, output_path)
    print()

    # Résumé final avec paramètres des modes
    print("=" * 70)
    print("📋 RÉSUMÉ FINAL - SUISSE VAGUE 1")
    print("=" * 70)
    print()

    if best_model:
        print(f"🏆 MEILLEUR MODÈLE ({best_n_modes} modes):")
        mode_names = ['Zurich/Genève', 'Cantons urbains', 'Régional    ', 'Alpin       ']
