#!/usr/bin/env python3
"""
Analyse COVID-19 Allemagne - Première Vague
...
def load_germany_data_github():
    """Télécharge les données COVID-19 Allemagne depuis GitHub JHU CSSE."""
    url = "https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_deaths_global.csv"

    print("📥 Téléchargement des données Allemagne depuis GitHub JHU CSSE...")
    try:
        df = pd.read_csv(url)
        # Allemagne - somme de toutes les régions
        germany_data = df[df['Country/Region'] == 'Germany'].iloc[:, 4:].sum(axis=0)
        germany = pd.DataFrame({'deaths': germany_data})
        germany.index = pd.to_datetime(germany.index)

        # Filtrer Vague 1
        start_date = '2020-02-15'
        end_date = '2020-06-30'
        germany = germany.loc[start_date:end_date]

        # Calculer décès quotidiens
        daily_deaths = germany['deaths'].diff().fillna(0)
        ...
        print(f"✅ Données Allemagne chargées: {len(t_data)} points")
...
def plot_comparison(t_data, y_data, dates, models_dict, output_path):
    ...
    fig.suptitle('COVID-19 Allemagne Vague 1 - Comparaison Super-Radiant vs SIR',
    ...
    if models_dict['sr4'] is not None:
        ...
        mode_names = ['Mode 1 (Urbain)', 'Mode 2 (Péri-urbain)', 'Mode 3 (Rural)', 'Mode 4 (Isolé)']
    ...
    ax3.set_title('Décomposition en 4 modes sociaux', fontsize=12, fontweight='bold')
...
def main():
    ...
    print("=" * 70)
    print("  ANALYSE COVID-19 ALLEMAGNE VAGUE 1")
    print("  Super-Radiant (sech²) vs SIR Standard")
    print("=" * 70)
    print()

    # [1] Chargement des données
    print("[1/5] Chargement des données Allemagne...")
    t_data, y_data, dates, max_deaths_raw = load_germany_data_github()
    print()
    ...
    if best_model and sir_model:
        ...
        print(f"📊 RÉSULTATS ALLEMAGNE VAGUE 1:")
    ...
    print()
    print("✅ Analyse Allemagne terminée!")
    print("=" * 70)
