#!/usr/bin/env python3
"""
Analyse COVID-19 Autriche - Première Vague
Comparaison Super-Radiant (sech²) vs SIR Standard

CAS TEST: Comparaison avec Allemagne (culture germanique similaire)
Structure: Pays fédéral (9 Länder)
...
def load_austria_data_github():
    """Télécharge les données COVID-19 Autriche depuis GitHub JHU CSSE."""
    url = "https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_deaths_global.csv"

    print("📥 Téléchargement des données Autriche depuis GitHub JHU CSSE...")
    try:
        df = pd.read_csv(url)
        # Autriche - somme de toutes les régions
        austria_data = df[df['Country/Region'] == 'Austria'].iloc[:, 4:].sum(axis=0)
        austria = pd.DataFrame({'deaths': austria_data})
        austria.index = pd.to_datetime(austria.index)

        # Filtrer Vague 1
        start_date = '2020-02-15'
        end_date = '2020-06-30'
        austria = austria.loc[start_date:end_date]

        # Calculer décès quotidiens
        daily_deaths = austria['deaths'].diff().fillna(0)
        ...
        print(f"✅ Données Autriche chargées: {len(t_data)} points")
        ...
        print(f"   📍 Structure: Pays fédéral (9 Länder), culture germanique")
...
def plot_comparison(t_data, y_data, dates, models_dict, output_path):
    ...
    fig.suptitle('COVID-19 Autriche Vague 1 - Comparaison Super-Radiant vs SIR\n(Pays fédéral - 9 Länder)',
    ...
    if models_dict['sr4'] is not None:
        ...
        mode_names = ['Mode 1 (Vienne)', 'Mode 2 (Länder urbains)', 'Mode 3 (Régional)', 'Mode 4 (Alpin)']
    ...
    ax3.set_title('Décomposition en 4 modes (Länder)', fontsize=12, fontweight='bold')
...
def main():
    ...
    print("=" * 70)
    print("  ANALYSE COVID-19 AUTRICHE VAGUE 1")
    print("  Super-Radiant (sech²) vs SIR Standard")
    print("  Pays fédéral (9 Länder) - Culture germanique")
    print("=" * 70)
    print()

    # [1] Chargement des données
    print("[1/5] Chargement des données Autriche...")
    t_data, y_data, dates, max_deaths_raw = load_austria_data_github()
    print()
    ...
    if best_model and sir_model:
        ...
        print(f"📊 RÉSULTATS AUTRICHE VAGUE 1:")
    ...
    print()
    print("🔬 COMPARAISON AVEC ALLEMAGNE:")
    print("   Culture germanique similaire, structure fédérale")
    print("   Allemagne: SIR gagne (RMS 2.77%)")
    if improvement < 1.0:
        print("   Autriche: SR gagne → Différence de gestion COVID, pas culturelle")
    else:
        print("   Autriche: SIR gagne → Pattern similaire à l'Allemagne")
    print()
    print("✅ Analyse Autriche terminée!")
    print("=" * 70)
